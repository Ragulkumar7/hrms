      ...
      <style>
        {`
          ...
          .filter-dropdown {
            position: absolute; 
            top: 45px; 
            right: 0; /* Aligned to the far right of the button container */
            width: 260px; 
            background: white; 
          ...
          .filter-header {
            ...
            margin-bottom: 12px; font-weight: 700; font-size: 13px; color: var(--text-dark);
          }
          ...
          .filter-section label {
            ...
            color: var(--text-grey); margin-bottom: 4px; text-transform: uppercase;
          }
          .filter-section select, .filter-section input {
            width: 100%; padding: 8px; border-radius: 6px; 
            border: 1px solid var(--border-color); font-size: 13px; outline: none;
          }
          ...
          {showFilterDropdown && (
            <div className="filter-dropdown">
              <div className="filter-header">
                ...
                <button onClick={() => setFilterConfig({dept:'', role:'', location:''})}>Reset</button>
              </div>
              
              <div className="filter-section">
                <label>Department</label>
                <select 
                  value={filterConfig.dept} 
                  onChange={(e) => setFilterConfig({...filterConfig, dept: e.target.value})}
                >
          ...
          <table className="custom-table">
            ...
            <tbody>
              {employees
                .filter(emp => {
                  return (
                    (filterConfig.dept === '' || emp.dept === filterConfig.dept) &&
                    (filterConfig.role === '' || emp.role.toLowerCase().includes(filterConfig.role.toLowerCase()))
                  );
                })
                .map((emp) => (
                  <tr key={emp.id}>
